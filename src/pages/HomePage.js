...
const HomePage = () => {
  const [books, setBooks] = useState(staticBooks.slice(0, 6)); // Limito a 6 libros por defecto
  ...
  const showAddBookTab = () => {
    setIsAddingBook(true);
  };

  const showBookListTab = () => {
    setIsAddingBook(false);
  };
  ...
  return (
    <div className="homepage">
      ...
      <div className="tab-buttons">
        <button onClick={showBookListTab} className={!isAddingBook ? 'active' : ''}>Ver Libros</button>
        <button onClick={showAddBookTab} className={isAddingBook ? 'active' : ''}>Agregar Libro</button>
      </div>
      
      {/* Contenido de la pestaña de Agregar Libro */}
      {isAddingBook && (
      ...
      {/* Contenido de la pestaña de Ver Libros */}
      {!isAddingBook && (
